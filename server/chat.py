...
EDIT_GRAMMAR_NOTE_TOOL = {
    "name": "edit_grammar_note",
    "description": (
        "Виправляє вже ІСНУЮЧУ нотатку з граматики — заголовок і/або зміст. "
        "Нотатка шукається за id (видно у списку наявних нотаток у системному "
        "промпті). Використовуй, коли учень вказує на неповноту чи помилку в "
        "раніше збереженій нотатці (напр. 'ти не все додав', 'тут помилка') — "
        "НЕ для нових нотаток (для цього save_grammar_note, бо повторне "
        "збереження створило б дублікат замість виправлення наявної). "
        "Передавай лише те поле, яке справді треба виправити — решта лишається як є."
    ),
    "input_schema": {
        "type": "object",
        "properties": {
            "id": {"type": "integer", "description": "id нотатки (зі списку наявних нотаток)"},
            "title": {"type": "string", "description": "Новий заголовок (необов'язково)"},
            "content": {"type": "string", "description": "Новий зміст (необов'язково)"},
        },
        "required": ["id"],
    },
}
...
def execute_edit_grammar_note(conn, tool_input):
    try:
        note_id = int(tool_input.get("id"))
    except (TypeError, ValueError):
        return {"ok": False, "error": "невалідний id"}
    ...
    if not title and not content:
        return {"ok": False, "error": "потрібне хоча б одне з: title, content"}
    row = conn.execute(
        "SELECT id FROM grammar_notes WHERE id = ?", (note_id,)
    ).fetchone()
    if row is None:
        return {"ok": False, "error": "нотатку не знайдено"}
    updates = {}
    ...
    set_clause = ", ".join(f"{col} = ?" for col in updates)
    conn.execute(
        f"UPDATE grammar_notes SET {set_clause} WHERE id = ?",
        (*updates.values(), note_id),
    )
    conn.commit()
    return {"ok": True, "id": note_id, **updates}


CHAT_TOOLS = [
    ADD_WORD_TOOL, RETAG_WORD_TOOL, EDIT_WORD_TOOL,
    SAVE_GRAMMAR_NOTE_TOOL, GET_GRAMMAR_NOTE_TOOL, EDIT_GRAMMAR_NOTE_TOOL,
]
CHAT_TOOL_EXECUTORS = {
    ...
    "edit_grammar_note": execute_edit_grammar_note,
}
...
def _tool_call_summary(name, tool_input, result):
    ...
    elif name in ("get_grammar_note", "edit_grammar_note"):
...
посилання.
- Якщо учень каже, що в раніше збереженій нотатці бракує інформації чи є помилка (напр. \
"ти не все додав", "тут неправильно") — використай edit_grammar_note (id зі списку нижче), \
а НЕ повторний save_grammar_note (це створило б дублікат замість виправлення наявної). \
Передавай лише те поле (title і/або content), яке справді треба виправити.
